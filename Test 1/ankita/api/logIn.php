...
try{
$checkVaildLogInId = $pdo->prepare("select id,username,email,phone from users where email=? and password=? ");
...
$isValidLogInId= $checkVaildLogInId->fetchAll(PDO::FETCH_ASSOC);
   ...
   $addToken=$pdo->prepare('insert into session_table (users_id,user_tokan) values(?,?)');
