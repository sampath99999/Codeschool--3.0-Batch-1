...
--creating table for working status
CREATE TABLE working_status(
    id int NOT NULL AUTO_INCREMENT PRIMARY KEY,
    work_description varchar(50)
);
--creating table for designation
use employees_database;
...
--creating table for salary_components
use employees_database;
...
--creating table for salary_details
...
VALUES (1, 11, 2023, '2023-11-03', now(), 2800, 120, 2680),
...
SELECT CONCAT(e.first_name,e.last_name) FullName, e.date_of_joining, e.date_of_birth, e.gender, e.phone_no, w.work_description, d.designation_description, l.district  
from Employees e 
LEFT JOIN working_status w 
ON e.working_status_id=w.id 
INNER JOIN designation d 
ON e.designation_id=d.id
INNER JOIN location_details l 
ON e.location_id=l.id;

---2.count of employees in location wise(location,employee count), 

SELECT  l.district,count(e.id) FROM employees e INNER JOIN location_details l ON e.location_id=l.id GROUP BY district;


--3.count of employees in designation wise (designation,employee count)

SELECT  d.designation_description,count(e.id) FROM employees e INNER JOIN designation d ON e.designation_id=d.id GROUP BY designation_description;


--4. employees retirement date (empoyee_id, employee_name, dob, doj, retirement_date)

SELECT  w.work_description,count(e.id) FROM employees e INNER JOIN working_status w ON e.working_status_id=w.id GROUP BY work_description;


--5. employees retirement date (empoyee_id, employee_name, dob, doj, retirement_date)

SELECT id,CONCAT(first_name,last_name) FullName,date_of_birth,date_of_joining,CONCAT(YEAR(date_of_birth)+60,'-',MONTH(date_of_birth),'-',DAY(date_of_birth))  retirement_date FROM employees;


--6.salary received by each employee for last month (employee_id, name, salary_year, salary_month, gross, deduction, net).

SELECT  e.id,CONCAT(e.first_name,e.last_name) FullName ,s.salary_year,s.salary_month,e.gross,s.deduction,s.net FROM employees e LEFT JOIN salaries s ON e.id=s.employee_id WHERE s.salary_month =MONTH(CURRENT_DATE())-1 AND s.salary_year=YEAR(CURRENT_DATE());

--7.maximum, minimum salary of employee, average salary of employees.
`   `
SELECT min(gross) min_salary,max(gross) max_salary,avg(gross) avg_salary  FROM Employees;

--8.show salary break up(each salary component) of each employee wise.

 SELECT s.employee_id,CONCAT(e.first_name,e.last_name) FullName,sc.salary_description,sd.amount FROM salaries s LEFT JOIN salary_details sd ON s.id=sd.salaries_id INNER JOIN employees e ON s.employee_id=e.id INNER JOIN salary_components sc ON  sd.salary_component_id=sc.id ORDER BY s.employee_id;

--9.list of emp who have not received salary for last month.

SELECT  e.id,CONCAT(e.first_name,e.last_name) FullName FROM employees e LEFT JOIN salaries s ON e.id=s.employee_id WHERE s.salary_month !=MONTH(CURRENT_DATE())-1 AND s.salary_year!=YEAR(CURRENT_DATE()) GROUP BY id;
