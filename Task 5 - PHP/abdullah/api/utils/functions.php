...
function generateTokenForUser($pdo, $length = 10)
{
    ...
    if ($stmt->rowCount() > 0) {
        $randomString = generateTokenForUser($pdo, $length);
    }
    return $randomString;
}

function generateTokenForAdmin($pdo, $length = 10)
{
    $characters = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
    $charactersLength = strlen($characters);
    $randomString = '';
    ...
    if ($stmt->rowCount() > 0) {
        $randomString = generateTokenForAdmin($pdo, $length);
    }
    return $randomString;
}
