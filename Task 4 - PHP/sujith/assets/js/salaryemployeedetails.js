...
function DisplayData(users){
    ...
    users.forEach((user)=>{
        ...
        return(
            ...
            <tr class="bg-white">
                ...
                <td class="py-3 text-center">${convertMonth(user.salary_month)}</td>
                ...
                <button type="button" class="btn btn-success btn-sm " data-bs-toggle="modal" data-bs-target="#viewComponentModal" id="${user.id}"   onclick="getSingleSalaryComponent(this)">
                View
              </button>
              </tr>
           `
  ...
  function getSingleSalaryComponent(ele){
    ...
    $.ajax({
        url: "./api/getSingleSalaryComponent.php",
        method: "POST",
        data: {
            id,
        },
...
function addSalary(){
    ...
    $.ajax({
        ...
        success:function(response){
            response=JSON.parse(response);
            let id = response.data.salaries[0].id;
